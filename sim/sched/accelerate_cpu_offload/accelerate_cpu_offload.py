...
class AccelerateCPUOffload(BaseScheduler):
    """HuggingFace ``accelerate.cpu_offload`` policy for PyTorch-profile traces.

    Forked from ``DeviceAwareVanillaAsync``: identical device-aware routing,
    multi-phase layout, lifetime tracking, and async H2D/D2H transfer
    machinery. The difference is that this scheduler computes the
    accelerate cpu_offload schedule *itself* in ``compile()`` (retargeting
    weight tensors to a RAM home and planning per-weight prefetch/evict)
    instead of reading ``xfer_arrivals`` / ``evict_after_node`` marks
    injected offline by ``graph_modifiers`` — so it is self-contained.

    accelerate ``cpu_offload(model, execution_device)`` keeps one CPU copy
    of every weight and, per submodule, copies that module's weights
    CPU->GPU just before its forward (``AlignDevicesHook.pre_forward``) and
    frees the GPU copy right after (``post_forward``, ``offload=True``).
    Inference weights are read-only, so eviction is a free, not a D2H copy.
    Transfers are synchronous on the default stream (no prefetch overlap),
    which is why it is far slower than the resident run but holds only one
    module's weights in VRAM at a time.
    """
    ...
    def compile(self, trace: Trace) -> None:
        """Compute the accelerate cpu_offload schedule and write it into the
        Trace as ``graph_modifiers``-style marks, then rebuild the inherited
        DAV indices off those marks.

        The raw PyTorch-profile bundle for a cpu_offload run does NOT carry
        clean WEIGHT tensors: the caching allocator re-streams each weight to
        a fresh GPU buffer every forward, so the loader records each use as a
        separate ``tensor_kind=='CONTEXT'`` reincarnation. There are ~196 CPU
        masters (the model resident in RAM, ~5.6 GB) plus tens of thousands of
        per-forward cuda reincarnations totalling >150 GB — far past VRAM, so a
        vanilla layout aborts.

        This pass replays ``graph_modifiers`` storage-coalesce + hf_accelerate
        ``model`` policy in-scheduler:

          1. Resolve each weight's owner module (gpu nodes carry no
             ``module_path``; walk to the CPU dispatcher via
             ``start_gated_edges`` and up its parent chain).
          2. Coalesce every no-producer (INPUT) cuda CONTEXT reincarnation onto
             a single CPU master per logical weight (owner, with a shape
             fallback). Redirect all consumers to the master; neutralise the
             orphan reincarnations (retype INTERMEDIATE) so layout skips them.
          3. Mark every master ``evictable`` and emit ``evict_after_node`` so
             its VRAM copy is freed right after each gpu consumer retires.

        No ``xfer_arrivals`` are emitted: accelerate cpu_offload is synchronous
        (``AlignDevicesHook`` copies in ``pre_forward``, frees in
        ``post_forward``, default stream — no prefetch overlap), so the
        inherited ``_ensure_inputs_resident`` on-demand RAM->VRAM staging — which
        makes the consumer wait for its load — is exactly the right model.
        """
        nm = trace.node_map
        tm = trace.tensor_map
        ...
        def is_cuda(t) -> bool:
            return str(t.args.get("device", "")).lower().startswith("cuda")

        # ---- 1. gpu -> cpu dispatcher map + module-path resolver ----
        g2c: dict[int, int] = {}
        ...
        def module_path_of(node_id: int) -> str | None:
            ...
            if mp:
                return mp
            # gpu nodes carry no module; hop to the CPU dispatcher then walk
            # up its parent chain (cudaLaunchKernel -> aten op with a module).
            cur = g2c.get(node_id)
            hops = 0
        ...
        # ---- consumers index (pre-redirect) ----
        consumers: dict[int, list[int]] = {}
        for n in nm.values():
            for tid in n.input_tensors:
                consumers.setdefault(tid, []).append(n.id)

        # A streamable weight is a ``CONTEXT`` tensor that is "parameter
        # shaped": every dimension is large. The offload trace also tags some
        # activation / KV-cache buffers as ``CONTEXT``, but those carry a small
        # sequence or batch dimension (e.g. [9, 8192]) and are mutated in
        # place. Treating a mutated buffer as a read-only streamed master would
        # invalidate its RAM copy on write and then strand every later consumer
        # once the VRAM copy is evicted (deadlock). Real weights
        # ([8192, 3072], [128256, 3072], ...) have no small dimension, so a
        # min-dimension threshold cleanly separates the two.
        ...
        def group_key(t):
            ...
            if len(ow) == 1:
                # (owner, shape): owner is a specific module (e.g.
                # ...layers.15.mlp.up_proj) so this is one logical weight;
                # shape guards against an unrelated tensor sharing the owner.
                return (next(iter(ow)), shape)
            # multi-owner (shared/tied) or unresolved -> fall back to shape so
            # reincarnations of the same logical weight still coalesce.
            return ("shape", shape, tbytes(t))

        # ---- 2. designate one CPU master per group (prefer existing CPU
        ...
        master_map: dict[int, int] = {}   # reincarnation tid -> master tid
        streamed_masters: set[int] = set()
        orphans: list[int] = []
        for t in list(tm.values()):
            if not is_weight_tensor(t):
                continue
            if t.args.get("tensor_type") != "INPUT" or not is_cuda(t):
                continue
            key = group_key(t)
            master = owner_master.get(key)
            if master is None:
                # No CPU master for this group: promote this reincarnation to
                # the RAM-resident master (retarget cuda -> cpu).
                t.args["device"] = "cpu"
                owner_master[key] = t.id
                streamed_masters.add(t.id)
                continue
            master_map[t.id] = master
            streamed_masters.add(master)
            orphans.append(t.id)

        # ---- 3. redirect every consumer onto the master, dedup ----
        for n in nm.values():
        ...
        # ---- neutralise orphan reincarnations so layout never places them ----
        for tid in orphans:
            tm[tid].args["tensor_type"] = "INTERMEDIATE"

        # ---- 4. evict each master's VRAM right after every gpu consumer ----
        ...
        # ---- write graph_modifiers-style marks into the Trace ----
        ...
        self._acco_stats = {
            ...
            "coalesced_reincarnations": len(orphans),
