...
class DiffusersGroupOffload(BaseScheduler):
    """diffusers ``apply_group_offloading`` (leaf_level + use_stream) policy.

    Forked from ``DeviceAwareVanillaAsync`` for its transfer + ready-queue
    machinery, but takes the **approach-A / D-1** path: the offload run's own
    profiled trace already contains the real ``Memcpy HtoD`` transfers as
    explicit nodes, so this scheduler turns each such node into a genuine
    cg-sim ``TransferJob`` (RAM master -> VRAM copy) rather than replaying its
    recorded ``compute_time`` as a fixed compute cost. The transfer's wall time
    is therefore computed by cg-sim's water-filling bandwidth model
    (bytes / ``memory_bandwidth``) — so it *responds* to hardware/placement
    changes, which is the whole point of the simulator. The recorded Memcpy
    duration is used only to **calibrate** ``memory_bandwidth`` (size / duration
    ~= the real effective rate), never as the transfer time itself.

    Generic across diffusion models (keys on PyTorch-universal signals —
    ``Memcpy HtoD/DtoH`` op names, tensor device/kind, parameter shape, and
    module-path dispatcher resolution — never model-specific node names), so
    the same scheduler handles sdxl-turbo, sd3, etc.
    """
    ...
    def compile(self, trace: Trace) -> None:
        """diffusers group-offload (leaf_level) schedule, computed in-scheduler.

        cg-sim's ``TransferJob`` moves a *single tensor* between hardware
        (``src.tensor_id == dst.tensor_id``; ``transfer_assertion.py``), so the
        real H2D — recorded as ``Memcpy`` gpu nodes copying a RAM master into a
        *fresh* cuda reincarnation (a different tensor id) — cannot be replayed
        as a TransferJob verbatim. So we coalesce, exactly as the accelerate
        scheduler does: every streamed cuda weight reincarnation is mapped onto
        its one RAM master, all consumers are redirected to that master, and the
        explicit ``Memcpy`` nodes are neutralised (``compute_time`` -> 0). The
        inherited on-demand staging then moves the master RAM->VRAM as a genuine
        *same-tid* ``TransferJob`` — bytes / ``memory_bandwidth`` via
        water-filling, real IO that responds to hw/placement changes — and
        ``evict_after_node`` frees the VRAM copy after each gpu use. The
        recorded Memcpy duration is used only to calibrate ``memory_bandwidth``
        (the YAML), never as the transfer's wall time.

        Differences from accelerate (which this otherwise mirrors): masters here
        are ``WEIGHT``-kind (accelerate's were ``CONTEXT``), reincarnations are
        produced ``INTERMEDIATE`` (accelerate's were producer-less ``INPUT``),
        and the trace carries explicit ``Memcpy`` nodes to neutralise. Generic
        across diffusion models — keyed on tensor kind/device, parameter shape,
        ``Memcpy`` op names, and module-path dispatcher resolution, never on
        model-specific node names — so the same scheduler handles sd3 too.
        """
        nm = trace.node_map
        tm = trace.tensor_map
        # Unused here (the engine's same-tid TransferJob rules out a verbatim
        # Memcpy->transfer); kept empty so the inherited dispatch/retire hooks
        # no-op.
        self._memcpy_plan = {}
        self._memcpy_jobs = {}

        def tbytes(t) -> int:
            return getattr(t, "size_bytes", None) or t.num_pages * 4096

        def device_of(tid: int) -> str | None:
            t = tm.get(tid)
            if t is None:
                return None
            d = str(t.args.get("device", "")).lower()
            return "cuda" if d.startswith("cuda") else d

        min_weight_dim = int(self.args.get("offload_min_weight_dim", 64))

        def is_weight_tensor(t) -> bool:
            # diffusers labels offloaded params WEIGHT or CONTEXT; both count.
            if t.args.get("tensor_kind") not in ("WEIGHT", "CONTEXT"):
                return False
        ...
        # ---- gpu -> cpu dispatcher map + module-path resolver ----
        g2c: dict[int, int] = {}
        for parent_id, child_id in (trace.args.get("start_gated_edges") or []):
            g2c[int(child_id)] = int(parent_id)

        def module_path_of(node_id: int) -> str | None:
            node = nm.get(node_id)
            if node is None:
                return None
            mp = (node.args.get("module") or {}).get("module_path")
            if mp:
                return mp
            cur = g2c.get(node_id)
            hops = 0
            while cur is not None and hops < 12:
                ...
                if cn is None:
                    break
                mp = (cn.args.get("module") or {}).get("module_path")
                if mp:
                    return mp
                cur = cn.parent_nodes[0] if cn.parent_nodes else None
                hops += 1
            return None
        ...
        for n in nm.values():
            for tid in n.input_tensors:
                consumers.setdefault(tid, []).append(n.id)

        def owner_of(tid: int) -> set[str]:
            paths: set[str] = set()
            for cid in consumers.get(tid, []):
                cn = nm[cid]
                if (cn.args.get("runtime_role") or "") == "gpu_runtime":
                    mp = module_path_of(cid)
                    if mp:
                        paths.add(mp)
            return paths
        ...
        def group_key(t):
            ow = owner_of(t.id)
            shape = tuple(t.args.get("shape") or ())
            if len(ow) == 1:
                return (next(iter(ow)), shape)
            return ("shape", shape, tbytes(t))

        # ---- map reincarnation -> master DIRECTLY from the Memcpy HtoD nodes ----
        # Each Memcpy HtoD copies a RAM master (cpu src) into a cuda
        # reincarnation (dst): that IS the real H2D linkage. (Owner-resolution
        # via module_path fails here — a master's only gpu consumer is the
        # Memcpy node, whose module resolves differently from the matmul that
        # consumes the reincarnation, so they never share a group key.) Restrict
        # the src to a genuine parameter-shaped master so the trace's 0-byte
        # placeholder srcs are ignored.
        min_master_bytes = int(self.args.get("offload_min_master_bytes", 4096))
        master_map: dict[int, int] = {}
        streamed_masters: set[int] = set()
        for n in nm.values():
            if "Memcpy HtoD" not in (n.args.get("op_name") or ""):
                continue
            tids = list(n.input_tensors)
            for t in n.output_tensors:
                if t not in tids:
                    tids.append(t)
            cpu_t = [t for t in tids if device_of(t) == "cpu"]
            cuda_t = [t for t in tids if device_of(t) == "cuda"]
            if not cpu_t or not cuda_t:
                continue
            src, dst = cpu_t[0], cuda_t[0]
            mt = tm.get(src)
            if src == dst or mt is None or not is_weight_tensor(mt) or tbytes(mt) < min_master_bytes:
                continue
            master_map[dst] = src
            streamed_masters.add(src)

        # ---- neutralise the explicit Memcpy transfer nodes: the master's
        #      same-tid staging transfer now simulates the H2D; their recorded
        #      compute_time would otherwise double-count it as fake compute. ----
        for n in nm.values():
            op = n.args.get("op_name") or ""
            if "Memcpy HtoD" in op or "Memcpy DtoH" in op:
                n.compute_time_micros = 0.0

        # ---- redirect every consumer onto the master (dedup) ----
        for n in nm.values():
            if not n.input_tensors:
                continue
            changed = False
            new_in: list[int] = []
            seen: set[int] = set()
            for tid in n.input_tensors:
                m = master_map.get(tid, tid)
                if m != tid:
                    changed = True
                if m not in seen:
                    seen.add(m)
        ...
        # ---- bypass residency on the neutralised Memcpy nodes and on any CPU
        #      op that still reads a cuda tensor. CRITICAL: do NOT bypass the
        #      real gpu compute (matmul) nodes — after coalescing they consume
        #      the cpu master, and they MUST go through _ensure_inputs_resident
        #      so it stages the master RAM->VRAM as a real TransferJob (the H2D
        #      IO). A blanket cross-device bypass here silently skips that
        #      staging and the weights never move. So bypass only:
        #        - Memcpy nodes (0-cost markers; their master input must not be
        #          re-staged), and
        #        - cpu-compute nodes with a cuda input (assignment/metadata ops
        #          the engine can't satisfy by staging onto the CPU).
        for n in nm.values():
            if n.custom_deps:
                continue
            op = n.args.get("op_name") or ""
            is_memcpy = "Memcpy HtoD" in op or "Memcpy DtoH" in op
            compute_dev = "cuda" if str(n.args.get("device_type", "")).upper() in ("CUDA", "GPU") else "cpu"
            has_cuda_input = any(device_of(tid) == "cuda" for tid in n.input_tensors)
            if is_memcpy or (compute_dev == "cpu" and has_cuda_input):
                for parent_id in n.parent_nodes:
                    n.custom_deps.append(NodeDoneDep(parent_id))

        # ---- evict each master's VRAM after every gpu consumer (free, not D2H —
        #      diffusers leaf_level eviction is a pointer-swap; the trace shows
        #      ~0 D2H bytes) ----
        evict_after: dict[int, list[int]] = {}
        for n in nm.values():
            if (n.args.get("runtime_role") or "") != "gpu_runtime":
                continue
            ms = sorted({tid for tid in n.input_tensors if tid in streamed_masters})
            if ms:
                evict_after[n.id] = ms

        trace.args["evict_after_node"] = evict_after
        trace.args["evictable_tensor_ids"] = set(streamed_masters)

        # ---- rebuild inherited DAV indices off the rewritten graph + marks ----
        self._evictable_tensor_ids = set(streamed_masters)
        self._remaining_consumers.clear()
        self._remaining_cpu_consumers.clear()
        self._consumers_by_tid.clear()
        ...
        self._dgo_stats = {
            "masters": len(streamed_masters),
            "coalesced_reincarnations": len(master_map),
            "evict_nodes": len(evict_after),
            "master_bytes": sum(tbytes(tm[m]) for m in streamed_masters),
        }
        return

    # ===================================================== memcpy -> transfer
    def _issue_memcpy_transfer(self, node: Node) -> bool:
        """Dispatch a real cg-sim TransferJob for a recorded Memcpy node:
        src master region -> dst copy region. Returns False (retry next tick)
        if the source isn't readable yet or no idle dst region is available."""
        src_tid, dst_tid, kind = self._memcpy_plan[node.id]
        dst_tensor = self.sys.trace.tensor_map.get(dst_tid)
        if dst_tensor is None:
            return False
        src_mem, dst_mem = (self._ram, self._vram) if kind == "h2d" else (self._vram, self._ram)
        src = next((r for r in src_mem.space.get_by_tensor_id(src_tid) if self._region_readable(r)), None)
        if src is None:
            return False
        dst = self._find_or_claim_dest_region(dst_mem, dst_tensor)
        if dst is None or dst.access_status != DataRegionAccess.IDLE:
            return False
        job_id = self.sys.transfer([(src, dst)])
        self._memcpy_jobs[job_id] = node.id
        return True

    def _handle_memcpy_retires(self, retired_jobs: list[BaseJob]) -> None:
        """When a Memcpy node's TransferJob retires, mark the node DONE and
        release its control children (mirrors `_retire_completed_nodes`)."""
        for job in retired_jobs:
            if not isinstance(job, TransferJob):
                continue
            nid = self._memcpy_jobs.pop(job.id, None)
            if nid is None:
                continue
    ...
    def _dispatch_ready_memcpy_nodes(self) -> bool:
        """Pull ready Memcpy nodes out of the queue and dispatch them as
        TransferJobs instead of ComputeJobs."""
        if not self._memcpy_plan:
            return False
        issued = False
        keep: deque[int] = deque()
        for _ in range(len(self.ready_node_ids)):
            nid = self.ready_node_ids.popleft()
            if nid not in self._memcpy_plan:
                keep.append(nid)
                continue
            node = self.sys.trace.node_map[nid]
            if node.status != NodeStatus.TODO:
                continue
            if (self.pending_parent_count.get(nid, 0) != 0
                    or self._pending_started_count.get(nid, 0) != 0
                    or not self._parents_satisfied(node)):
                keep.append(nid)
                continue
            if self._issue_memcpy_transfer(node):
                node.status = NodeStatus.WAITING
                issued = True
            else:
                keep.append(nid)
        keep.extend(self.ready_node_ids)
        self.ready_node_ids = keep
        return issued
    ...
    def _submit_ready_nodes(self) -> bool:
        ...
        """
        # Approach-A: recorded Memcpy nodes become real TransferJobs, not
        # ComputeJobs. Pull them out before the compute submit loop.
        memcpy_issued = self._dispatch_ready_memcpy_nodes()
        if not self._gate_by_consumer:
            return self._submit_ready_nodes_core() or memcpy_issued
        ...
        return any_submitted or memcpy_issued
    ...
    def runtime(self, retired_jobs: list[BaseJob]) -> None:
        ...
        self._handle_memcpy_retires(retired_jobs)
