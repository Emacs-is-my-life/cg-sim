...
_SERVER_INSTRUCTIONS = (
    ...
    "  Two optional callables on every Node let user code run around\n"
    "  that node's ComputeJob without editing the engine/scheduler:\n"
    "    node.hook_pre_run:  Callable[[System], None] | None\n"
    "      Fires just *before* the ComputeJob's `begin(...)`. Mutations\n"
    "      here (tensor sizes, hardware state, trace bookkeeping) are\n"
    "      visible to begin(), so they affect the current node's\n"
    "      compute/transfer ETA estimate.\n"
    "    node.hook_post_run: Callable[[System], None] | None\n"
    "      Fires just *after* the ComputeJob retires, *before*\n"
    "      BREAK_AT_JOB_RETIRED parks the run — so at that breakpoint\n"
    "      the observer sees post-hook mutations already applied\n"
    "      (symmetric with BREAK_AT_JOB_DISPATCHED + pre-hook).\n"
    "      Mutations here propagate to downstream nodes only; the\n"
    "      just-retired job's duration is already final.\n"
    "  Both receive only `System` (== engine.sys) — exposing sys.trace\n"
    "  (`node_map`, `tensor_map`) and sys.hw (`dict[str, BaseHardware]`).\n"
    "  Engine/Debugger/Job are *not* passed; capture them by closure at\n"
    "  attach time if needed. Only ComputeJobs trigger hooks. Exceptions\n"
    "  in hooks propagate uncaught — with BREAK_ON_EXCEPTION on (default),\n"
    "  you land at `break_on_exception[<Type>]` with exception_origin\n"
    "  pointing to the hook line.\n"
    "\n"
    "  Use cases: dynamic weight sparsity, runtime quantization, KV-cache\n"
    "  eviction, fault injection — anything that mutates workload or\n"
    "  hardware state mid-run. Attach via `execute(...)` at any breakpoint\n"
    "  where the target Node is reachable (e.g. break_after_compile_stage):\n"
    "    execute(\"\"\"\n"
    "    def shrink(sys, _dbg=debug):\n"
    "        t = sys.trace.tensor_map[<tid>]\n"
    "        _dbg.record({'hook':'shrink','tid':<tid>,'old':t.num_pages})\n"
    "        t.num_pages //= 2\n"
    "    trace.node_map[<node_id>].hook_post_run = shrink\n"
    "    \"\"\")\n"
    "  Hooks persist across `continue_simulation` — clear with\n"
    "  `node.hook_post_run = None` (or `restart_simulation`, which\n"
    "  rebuilds the Trace and drops all hooks).\n"
    "\n"
    "  WARNING: state-coupled mutations (e.g. tensor resizing visible to\n"
    "  a scheduler that already committed a layout) will silently produce\n"
    "  wrong results because scheduler-internal caches (slot sizes, page\n"
    "  indices, prefetch plans) are not re-derived. Safe pattern for that\n"
    "  case: write a *new* scheduler that attaches its own hooks in\n"
    "  `__init__`/`compile()` and reads the mutated state back in\n"
    "  `runtime()` — same mutator and consumer.\n"
    "\n"
