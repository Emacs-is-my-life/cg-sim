...
def _solve_milp(
    ...
    *,
    ...
    # ---- 2b. Channel job set (single serial H2D machine) ----
    #
    # Overlap-aware lateness model. The H2D channel is ONE serial server
    # (h2d_streams=1) carrying prefetch + refetch transfers; evictions are
    # free VRAM drops (not transfers), so they never enter here. We model
    # the channel as a single machine with RELEASE TIMES and DEADLINES and
    # take L = max lateness as the makespan extension. Unlike the old
    # per-window throughput proxy, this *values overlap*: a job with wide
    # slack [r, d] sequences before its deadline and costs nothing; only
    # transfers the channel can't fit before their deadline extend the run.
    #
    # One job per:
    #   - streamed initial prefetch (x = 1 − c_t), deadline = first consumer
    #   - chosen refetch         (x = e_{t,k}),  deadline = consumer_{k+1}
    #
    # release r_j = max(prior_use_end, deadline − W); W = lookahead horizon
    # (issue at most W ahead). W is the single overlap↔peak tradeoff knob:
    # the peak rows charge a streamed tid W of residency before each
    # consumer (see _pt_arc_tau below), so a wider W buys overlap at the
    # cost of peak. processing p_j = δ_t (per-tid H2D time). Both in ms.
    _timeline_start_ns = min(c[1] for pt in pool.values() for c in pt.consumers)
    _W_ns = max(1, int(lookahead_ns))
    # channel_jobs: (deadline_ms, var_col, is_complement, p_ms, r_ms)
    #   is_complement=True  → x = 1 − var (initial prefetch, var = c_t)
    #   is_complement=False → x = var     (refetch,           var = e_{t,k})
    channel_jobs: list[tuple[float, int, bool, float, float]] = []
    for tid in feasible_tids:
        pt = pool[tid]
        p_ms = pt.tau_h2d_ns / MODEL_SCALE
        d0 = pt.consumers[0][1]
        r0 = max(_timeline_start_ns, d0 - _W_ns)
        channel_jobs.append(
            (d0 / MODEL_SCALE, c_var_idx[tid], True, p_ms, r0 / MODEL_SCALE)
        )
        for k in range(len(pt.consumers) - 1):
            if (tid, k) not in e_var_idx:
                continue
            dk = pt.consumers[k + 1][1]
            rk = max(pt.consumers[k][2], dk - _W_ns)
            channel_jobs.append(
                (dk / MODEL_SCALE, e_var_idx[(tid, k)], False, p_ms,
                 rk / MODEL_SCALE)
            )
    ...
    # Overlap-aware lateness: a SINGLE slack L = max lateness on the serial
    # H2D channel = makespan extension (the channel-schedule rows below tie
    # it to the chosen c/e). Minimizing L rewards prefetches that overlap
    # compute (wide release→deadline slack ⇒ free) and penalizes only the
    # transfers the channel can't deliver in time — which is the entire
    ...
    # NOTE: the old option-(b) lateness→peak coupling is removed in this
    # variant. It tied a per-window stall slack into the peak rows; there
    # are no per-window slacks here (the channel model uses a single L), and
    # the coupling was mis-calibrated/harmful anyway. `lateness_peak_coupling`
    # is accepted for interface parity but ignored.
    _coupling_on = False
    if audit and arc_queue_factor != 1.0:
        print(
            f"[ct_milp_overlap:audit] arc widening: × {arc_queue_factor}"
        )
    ...
    # No per-window slacks in the channel model; backpressure (off by
    # default) gets empty lists.
    per_window_lateness_ns: list[int] = []
...
def _emit_neutral(
    ...
    for tid, pt in pool.items():
        ...
        if is_cold:
            ...
            # Issue ~W ahead (the lookahead horizon), not JIT, so the
            # transfer actually overlaps compute — matching the channel
            # model's release r = max(prior, deadline − W). Offset is at
            # least τ_h2d (need the transfer's own lead time).
            _issue_offset = max(int(pt.tau_h2d_ns), int(lookahead_ns))
            issue_nid, _issue_lp, issue_gid = _pick_issuer_node(
                c0_gid, c0_lp_start, _issue_offset,
            )
            if issue_nid < 0:
                issue_nid = c0_nid
                issue_gid = c0_gid
        ...
        for k in range(len(pt.consumers) - 1):
            ...
            re_nid, re_lp_ts, re_gid = _pick_issuer_node(
                kp1_gid, ckp1_lp_start,
                max(int(pt.tau_h2d_ns), int(lookahead_ns)),
