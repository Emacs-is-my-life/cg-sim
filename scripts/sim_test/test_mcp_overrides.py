...
FLEXINFER_YAML = REPO_ROOT / "examples" / "run" / "llamacpp__llama-3-8B__flexinfer.yaml"
VANILLA_YAML = REPO_ROOT / "examples" / "run" / "llamacpp__llama-3-8B__vanilla.yaml"
...
async def main() -> int:
    ...
    async with stdio_client(server) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            pw = await _exec_int(session, "engine.sched.prefetch_window")
            # YAML default is 5 (examples/run/llamacpp__llama-3-8B__flexinfer.yaml).
